...
void input_output_class::out_data(char const *comp_name, flcuda** out_value,int step_number, int number, int r_step,int z_step)
{
 ...
 // 	if (step_number/number)
   ...
   inc_value = step_number/number;
	...
	strcpy(st_name,comp_name);
	char str_int [100]; 
//	 (inc_value,str_int,10);
	sprintf(str_int,"%d",inc_value);
	strcat(st_name, str_int);
	char path[100] = "E:/Science_plasma/pdp3_result_____/____";
