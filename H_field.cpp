...
///////////////////////////////////////////////
	...
	for(int i=0;i<(geom1->n_grid_1-1);i++)
		for(int k=0;k<(geom1->n_grid_2-1);k++)
		{
			...
			alpha=((e_field1->e3[i+1][k]-e_field1->e3[i][k])/geom1->dr - (e_field1->e1[i][k+1]-e_field1->e1[i][k])/geom1->dz)/MAGN_CONST;
			this->h2_half_time[i][k] = this->h2[i][k]+alpha*time1->delta_t/2;
			this->h2[i][k] = this->h2[i][k]+alpha*time1->delta_t;

			alpha= ((e_field1->e2[i+1][k]+e_field1->e2[i][k])/(2.0*geom1->dr*(i+0.5))+(e_field1->e2[i+1][k]-e_field1->e2[i][k])/geom1->dr)/MAGN_CONST;
