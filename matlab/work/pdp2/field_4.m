function fi = field_3(rho, geometry, bc)
...
%
%  x_type, bcy_type - type of boundary conditions on
...
%the length of vectors which represent the values on the boundary depend on
...
% temp(1,1) is equal to 0; there is a stage of the algorithm when we will
...
disp('Real and imaginary parts of potential:')
sum(sum(real(fi)))
sum(sum(imag(fi)))
