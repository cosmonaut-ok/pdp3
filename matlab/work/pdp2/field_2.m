function [ex ey fi] = field_2(rho, bcx_type, bcy_type, bcl, bcr, bcu, bcd, dx, dy)
...
if (bcx_type == 1)&(bcy_type == 1)
    if (bcu(1) ~= bcl(1))
        disp('Warning: the fist value of bcu vector and the first value of bcl vector should coincide!');
    end
    up_left_value = (bcu(1) + bcl(1))/2;
    
    if (bcu(end) ~= bcr(1))
        disp('Warning: the last value of bcu vector and the first value of bcr vector should coincide!');
    end 
    up_right_value = (bcu(end) + bcr(1))/2;
    
    if (bcd(end) ~= bcr(end))
        disp('Warning: the last value of bcd vector and the last value of bcr vector should coincide!');
    end 
    down_right_value = (bcd(end) + bcr(end))/2;   
    
    if (bcd(1) ~= bcl(end))
        disp('Warning: the first value of bcd vector and the last value of bcl vector should coincide!');
    end 
    down_left_value = (bcd(1) + bcl(end))/2;  
...
if (bcx_type == 1)&(bcy_type == 2)
    bcu = bcu(2:end-1);
    bcd = bcd(2:end-1);
end

if (bcy_type == 1)&(bcx_type == 2)
    bcl = bcl(2:end-1);
    bcr = bcr(2:end-1);
end
...
if (bcx_type == 1)&(bcy_type == 1)
    bcu = cat(2,up_left_value,bcu,up_right_value);
    bcd = cat(2,down_left_value,bcd,down_right_value);
end
