...
row_number = 65 # cfg.get_row_by_radius(radius)
...
if (len(data_r) == 0 or len(data_z) == 0) and autoselect:
    for probe in cfg.probes:
        if (probe.type == 'frame') and (probe.r_start <= row_number) and (probe.z_start == 0) and (probe.r_end >= row_number) and (probe.z_end == cfg.number_z_grid):
            ...
            if probe.component == 'E_r': data_r = reader.get_frame_range('E_r', shape, start_frame, end_frame)[:, row_number]
            if probe.component == 'E_z': data_z = reader.get_frame_range('E_z', shape, start_frame, end_frame)[:, row_number]
