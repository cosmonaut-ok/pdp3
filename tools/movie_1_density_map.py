...
def run(config_file, clim_d, video_file=None,
    ...
    t_plot_name = r'$\mathbf{Density\enspace}\enspace(\rho)$'
    ...
    for i in cfg.particles:
        if i.name == specie:
            if not clim_d:
                middle_den = (i.left_density + i.right_density) / 2
                clim_d = [middle_den / 2, middle_den * 2]

    if not clim_d: clim_d = [0, 1e17] # fallback
    # calculate/update video file path
    video_file = os.path.join(os.path.dirname(config_file), 'density_movie.avi') if not video_file else video_file
    ...
    plot = PlotBuilder(cfg.number_z_grid, cfg.number_r_grid,
                       ...
                       aspect='equal', image_interpolation='bicubic', image_cmap='gray_r')
    ...
    plot.add_image(t_plot_name, initial_image, cmap=cmap, clim=clim_d)
    plot.add_colorbar(t_plot_name, ticks=clim_d, title=cbar_axis_label)
    ...
    with writer.saving(fig, video_file, cfg.figure_dpi):
        for i in range(start_data_set, end_data_set):
            ...
            sys.stdout.flush()
            if cfg.use_hdf5:
                data_t = reader.get_frame('density', i)
                ...
                if i % frame_step == 0:
                    plot.add_image(t_plot_name, data_t, cmap=cmap, clim=clim_d)
                ...
                data_t = reader.get_all_frames_in_ds('density/{}'.format(specie), shape, i)
                ...
                for t in data_t:
                    if (frame + i * cfg.frames_per_file) % frame_step == 0:
                        ...
                        if i % frame_step == 0:
                            plot.add_image(t_plot_name, t, cmap=cmap, clim=clim_d)
...
def main():
    ...
    parser.add_argument('--specie', type=str,
                        help='Particles specie to visualize. Default electrons',
                        default='electrons')
    ...
    parser.add_argument('--clim-d', type=str,
    ...
    if os.path.isfile(args.properties_path):
        ...
        clim_d = list(map(float, args.clim_d.split(':'))) if args.clim_d else None
...
# run(config_file, clim_d, clim_e_z, clim_rho_beam, video_file='field_movie.avi',
