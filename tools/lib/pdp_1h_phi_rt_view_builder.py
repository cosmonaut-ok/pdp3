...
class Pdp2ERTViewBuilder:
    def __init__(self, cfg):
        ...
        self.x_axis_label = r'$\mathit{R (m)}$'
        ...
        self.image_r_range = cfg.r_grid_count
    ...
    def setup_2e_view(self):
        ...
        self._plot_builder.x_tick_start = 0
        self._plot_builder.x_tick_end =  self._cfg.r_size
        ...
        self._plot_builder.x_plot_size = self.image_r_range
    ...
    def create_view_with_2_plots(self):
        ...
        cache_r_name = format('image_ert_r_%e-%e-%e#%d_%d' % (
            self.__start_time,
            self.__end_time,
        ...
        cache_z_name = format('image_ert_z_%e-%e-%e#%d_%d' % (
            self.__start_time,
            self.__end_time,
            self._cfg.step_interval,
            self._cfg.data_dump_interval,
        ...
        if (len(image_r) == 0 or len(image_z) == 0):
            ...
            for k in range(self.start_data_set, self.end_data_set+1):
                ...
                for t in range(tstart, tend):
                    ...
                    image_r.extend(self._cfg.get_frame_col_from_data(h_field_e_r, local_step, longitude_col))
                    image_z.extend(self._cfg.get_frame_col_from_data(h_field_e_z, local_step, longitude_col))
