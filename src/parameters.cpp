...
XMLElement* Parameters::try_first_child(XMLElement* element, const char* name, bool fail)
{
  XMLElement* f_child = element->FirstChildElement(name);
  if (f_child == nullptr && fail)
  {
...
const char* Parameters::try_atribute(XMLElement* element, const char* name, bool fail)
{
  const char *attribute = element->Attribute(name);
  if (attribute == nullptr && fail)
  {
...
void Parameters::init_probes ()
{
  ...
  while(probe_xml)
  {
    ...
    p_probe.specie = (char*)try_atribute(probe_xml, "specie", false);
